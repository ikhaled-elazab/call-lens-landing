...
**⚠ THE PALETTE IS NOT YOURS TO INVENT. INHERIT IT FROM THE PRODUCT.**

An earlier version of this brief asked for "a compact token system — 4–6 named colours with
hex values". That produced a page in warm cream `#F6F4EF` with an electric-blue and
terracotta accent pair, against a product that is violet on a cool tinted white. A prospect
who clicks through from the page to a demo sees a different piece of software. Worse, the
invented cream-plus-terracotta is the first of the three banned defaults below — inventing a
palette is *how* you land on one.

These are the shipping tokens, copied from `callLens-frontend/src/app/globals.css`. Use
these values. Do not retune them for the page.

The dark column is what the app serves under its `.dark` class. This page has no theme
toggle, so those values go under `@media (prefers-color-scheme: dark)` per the quality floor
below — same colours, different switch. Do not ship a `.dark` class nothing sets.

```
Light                              Dark
--ink-900   #1e1a2b  headings      #ededed
--ink-700   #443f55  body          (mix of fg/muted)
--ink-500   #6a6480  captions      #a0a0a0
--page      #f4f3f7  page bg       #000000   ← true black, not a blue-black
--card      #ffffff  raised        #1a1a1a
--border    #e7e5ee  hairlines     #242424
--primary   #7133e6  buttons/links #9f6ffb   (brand-400 in dark)
--brand-500 #8f54ff  large/UI accent
--brand-50  #f5f0ff  tinted fills
--playhead  #e08a1e  see below
--radius    0.625rem
```

⚠ **`--primary` is `#7133e6`, not `#8f54ff`.** The raw brand only reaches 4.3:1 on white;
the 600 step is what clears AA for white button text at 6.3:1. Using the prettier value for
a filled button fails the contrast floor below.

⚠ **`--playhead: #e08a1e` IS RESERVED, AND IT IS THE ONE COLOUR THAT ARGUES THE PRODUCT.**
In the app it means one thing only: the moment being listened to, and the moment a citation
points at. Never a status, never a warning, never decoration. That makes it the correct
accent for the signature timeline described below — the amber on the page and the amber in
the product are the same claim. Do not spend it on anything else.

Sentiment has its own scale, deliberately not the status colours (a negative-sentiment call
is not an error): positive `#17784b`, neutral `#64686f`, negative `#a8442f`. If you show the
illustrative call's sentiment, use these.

Then do the planning pass: with the palette fixed, write out the layout concept, the type
roles, and the one signature element the page will be remembered by. Check it against this
brief and revise anything that reads like the design you would produce for any B2B SaaS page.
...
**Typography is inherited too, and the last attempt silently dropped it.** The product runs
**Geist Sans** for text and **Geist Mono** for data. The previous page shipped
`ui-sans-serif, "Helvetica Neue", Arial` for its display face — which is not a display face,
and is a second way the page stopped looking like the product.

Embed both (the brief allows no CDN links, so subset and inline them as `@font-face` with
`data:` URIs, or the page will silently fall back to Arial again — check the rendered result,
do not assume).

⚠ **Timecodes, ratios and durations must be Geist Mono with `font-feature-settings: "tnum"`.**
They appear all over this page and they should read as instrument readings, not body copy.
Without `tnum` a column of timecodes visibly jitters, which is the opposite of the
precision this page is arguing for.

⚠ **Arabic needs `"Noto Sans Arabic"` in the same stack, not a `:lang(ar)` swap.** Font
fallback is per-GLYPH, so one declaration covers Latin-inside-Arabic and Arabic-inside-Latin
— which is exactly what the RTL block below is. The product does it this way for that reason;
a language-scoped swap misses the mixed cases and drops digits out of the tabular face.
...
⚠ The fixed palette above already rules out the first two — they are colour choices you no
longer get to make. So this constraint now bites on **layout and type**, which is where the
generic version actually lives: a centred hero with a subhead and two buttons, a
three-across icon-and-heading feature grid, evenly-sized cards in a 12-column rhythm. The
palette being right is not the same as the page being distinctive.
